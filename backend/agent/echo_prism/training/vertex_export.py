...
1. Legacy: {"input_text": "...", "output_text": "Thought: ...\nAction: ..."}
...
def _build_multimodal_example(step: dict, workflow_name: str) -> dict | None:
    """
    Build one message-based multimodal training example (2026 schema).
    Uses screenshot_url when available; otherwise returns None (fallback to text-only).
    """
    ...
    if not screenshot_url:
        return example  # Fallback to legacy format
    user_content: list[dict] = [
        {"type": "image", "image_url": {"url": screenshot_url}},
...
async def export_training_data(
    ...
    for ft_doc in ft_docs:
        ...
        for step_doc in steps_ref.stream():
            ...
            # Prefer multimodal when screenshot_url available
            example = _build_multimodal_example(step, workflow_name) or _build_training_example(step, workflow_name)
            if example:
