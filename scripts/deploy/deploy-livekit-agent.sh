...
# Required: LiveKit credentials, EchoPrism Agent URL (for /api/agent/tool), Google/Gemini for voice
# LiveKit Google plugin expects GOOGLE_API_KEY; we set it from GEMINI_API_KEY (same key).
# ECHO_LOG_LEVEL=INFO so Cloud Run logs show agent activity (use DEBUG for more detail)
LIVEKIT_ENV="LIVEKIT_URL=$LIVEKIT_URL,LIVEKIT_API_KEY=$LIVEKIT_API_KEY,LIVEKIT_API_SECRET=$LIVEKIT_API_SECRET,LIVEKIT_AGENT_SECRET=$LIVEKIT_AGENT_SECRET,ECHOPRISM_AGENT_URL=$ECHO_PRISM_AGENT_URL,GEMINI_API_KEY=$GEMINI_API_KEY,GOOGLE_API_KEY=$GEMINI_API_KEY,ECHO_LOG_LEVEL=${ECHO_LOG_LEVEL:-INFO}"
