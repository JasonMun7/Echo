...
```typescript
const customizedTools = await composio.tools.get(
  "default",
  {
    toolkits: ["github"],
  },
...
// Step 4: Execute using the toolkit version discovered in Step 1 (pin that value for stability)
const result = await composio.tools.execute("GITHUB_GET_REPO", {
  userId: "user_123",
  arguments: { owner: "composio", repo: "sdk" },
  version: githubToolkit.version,
});
```
...
result = composio.tools.execute(
    ...
    user_id="user_123",
    arguments={"owner": "composio", "repo": "sdk"},
    version=github_toolkit.version,
)
```
...
const result = await composio.tools.execute("GITHUB_GET_REPOSITORY", {
  // ❌ Wrong name
  userId: "user_123",
  arguments: { repo: "sdk" },
  version: "12082025_00",
...
// DON'T: Paste a version string without discovering it from the toolkit first
const result = await composio.tools.execute("GITHUB_GET_REPO", {
  userId: "user_123",
  arguments: { owner: "composio", repo: "sdk" },
  version: "12082025_00", // ❌ Arbitrary/stale — fetch `githubToolkit.version` or `composio manage toolkits info "github"` first
});
```
...
result = composio.tools.execute(
    ...
    user_id="user_123",
    arguments={"owner": "composio", "repo": "sdk"},
    version="12082025_00"  # ❌ Arbitrary/stale — use github_toolkit.version or composio CLI discovery first
)
```
