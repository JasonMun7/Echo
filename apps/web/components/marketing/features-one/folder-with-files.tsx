      ...
      >
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 overflow-hidden">
          {[0, 1, 2, 3, 4].map((i) => (
            <div key={i} className="relative h-px w-full">
              {/* Dashed line */}
              <div className="absolute inset-0 border-t border-dashed border-neutral-200" />
              {/* Pulse beam */}
              <motion.div
        ...
        <div className="relative z-10 flex items-end gap-12">
          ...
          >
            ...
            >
              {/* Folder Back */}
              <div className="absolute inset-0 rounded-lg bg-gradient-to-b from-amber-400 to-amber-500 shadow-sm">
                {/* Folder Tab */}
                <div
                  className="absolute left-1.5 rounded-t-sm bg-gradient-to-b from-amber-300 to-amber-400"
              ...
              {files.map((image, index) => {
                ...
                return (
                  <motion.div
                    key={index}
                    className="absolute top-1.5 left-1/2 origin-bottom overflow-hidden rounded bg-white shadow-sm ring-1 shadow-black/10 ring-black/10"
                    animate={{
              ...
              {/* Folder Front */}
              <motion.div
                className="absolute inset-x-0 bottom-0 h-[85%] origin-bottom rounded-lg bg-gradient-to-b from-amber-300 to-amber-400 shadow-sm"
                animate={{
              ...
              >
                <div className="absolute top-2 right-2 left-2 h-px bg-amber-200/50" />
              </motion.div>
            </div>
          </motion.div>
          ...
          >
            <div
              className="relative rounded-lg bg-gradient-to-b from-gray-50 to-gray-50 shadow-sm"
              style={{ width: 56, height: 72 }}
            >
              {/* Server top panel with vents */}
              <div className="absolute inset-x-1.5 top-1.5 h-3 rounded-sm bg-white shadow-sm ring-1 shadow-black/5 ring-black/5">
                <div className="flex h-full items-center justify-center gap-0.5">
                  {[0, 1, 2, 3, 4, 5].map((i) => (
                    <div key={i} className="h-1.5 w-px bg-neutral-600" />
                  ))}
                </div>
              </div>
              ...
              <div className="absolute right-1.5 bottom-1.5 left-1.5 flex flex-col gap-1">
                {[0, 1].map((i) => (
                  <div
                    key={i}
                    className="h-4 rounded-sm bg-white shadow-sm ring-1 shadow-black/5 ring-black/5"
                  >
                    <div className="mt-1 ml-1 h-0.5 w-3 rounded-full bg-neutral-300" />
                  </div>
                ))}
              </div>
            </div>
