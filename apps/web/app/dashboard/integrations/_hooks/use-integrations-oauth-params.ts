...
export function useIntegrationsOAuthParams(
  ...
  const { loadIntegrations, connectVaultIntegration, pendingVaultHandledRef } = options;
  ...
  useEffect(() => {
    ...
    if (error) {
      ...
      } else if (error === "invalid_request" && /connection is not enabled/i.test(desc)) {
        toast.error(
      ...
      } else if (error === "invalid_request" && /not active for authentication/i.test(desc)) {
        toast.error(
  ...
  }, [searchParams, loadIntegrations, connectVaultIntegration, pendingVaultHandledRef]);
}
