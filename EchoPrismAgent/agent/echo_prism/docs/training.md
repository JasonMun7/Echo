...
Training utilities for UI-TARS self-improvement: trace scoring, COCO export, and Vertex AI fine-tuning.

## Modules

- **trace_filter.py** — Two-pass trace scoring (rule-based + Gemini VLM); writes to `filtered_traces` collection
...
1. **Score trace** — After a run completes, `score_trace()` applies rule-based and VLM scoring; stores in `filtered_traces/{workflow_id}_{run_id}`.
