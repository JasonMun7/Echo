"""
Gemini Live bidirectional audio: `client.aio.live.connect` bridge to the app WebSocket.
...
from echo_prism_agent.constants import VOICE_INPUT_PCM_SAMPLE_RATE
...
async def run_voice_session(
    ...
    """
    Run EchoPrism voice: Live API session loop with WebSocket bridging.

    Router provides websocket and handle_tool_call for tool execution.
    """
    if not types:
                        ...
                        if msg["type"] == "websocket.receive":
                            if msg.get("bytes") is not None:
                                await live_session.send_realtime_input(
                                    audio=types.Blob(
                                        data=msg["bytes"],
                                        mime_type=f"audio/pcm;rate={VOICE_INPUT_PCM_SAMPLE_RATE}",
                                    )
                                )
